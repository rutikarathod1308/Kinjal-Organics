...
erpnext.accounts.SalesInvoiceController = class SalesInvoiceController extends erpnext.selling.SellingController {
	...
	make_sales_return() {
		// Collect unique delivery notes from items
let unique_delivery_notes = [...new Set((cur_frm.doc.items || [])
    .map(row => row.delivery_note)
    .filter(dn => dn))];

// Loop through unique delivery notes only
unique_delivery_notes.forEach(dn => {
    frappe.call({
        method: "kinjal_organics.public.py.delivery_value.delivery_value",
        args: {
            delivery_note: dn
        },
        callback: function(r) {
            console.log(`Delivery Note ${dn}: ${r.message.length} records`);

            if (r.message.length === 0) {
                frappe.throw(`Please return Delivery Note Against This ${dn} before generating a Sales Return.`);
            } else {
                frappe.model.open_mapped_doc({
